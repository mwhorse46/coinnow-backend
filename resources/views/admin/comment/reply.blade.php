                    ...
                    <form method="post" action="{{ route('comments.reply', $comment->id) }}"
                        ...
                        <div class="pl-lg-4 row flex justify-content-center">
                            <div class="d-flex align-items-center">
                                @if ($comment->is_chat == 0 && $comment->status == 0)
                                    <input type="checkbox" style="width: 40px; height: 40px;" name="status" id="status" />
                                @endif
                            </div>
                            <div class="col-md-6 form-group{{ $errors->has('reply') ? ' has-danger' : '' }}">
                                ...
                                <textarea name="reply" id="reply"
                                    ...
                                    rows="5" placeholder="{{ __('reply the comment') }}"
                                    autofocus>{{ old('reply', $comment->reply) }}</textarea>
                                @if ($errors->has('reply'))
